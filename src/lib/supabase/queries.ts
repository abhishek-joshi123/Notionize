...
import { folders, workspaces, collaborators, users, files} from "../../../migrations/schema";
import db from "./db"
...
export const getFiles = async (folderId: string) => {
  ...
  try {
    const results = (await db.select().from(files).orderBy(files.createdAt).where(eq(files.folderId, folderId))) as File[] | [];
    return { data: results, error: null };
  } catch (error) {
...
export const getPrivateWorkSpaces = async (userId : string) => {
  ...
  try {
    ...
    .where(and(notExists(db.select().from(collaborators).where(eq(collaborators.workspaceId, workspaces.id))), eq(workspaces.workspaceOwner, userId))) as workspace[];
...
export const getCollaboratingWorkspaces = async(userId: string) => {
  ...
  try {
      const collaboratedWorkspaces = (await db.select({
        ...
        logo: workspaces.logo,
        bannerUrl: workspaces.bannerUrl,
      }).from(users).innerJoin(collaborators, eq(users.id, collaborators.userId)).innerJoin(workspaces, eq(collaborators.workspaceId, workspaces.id)).where(eq(users.id, userId))) as workspace[];
