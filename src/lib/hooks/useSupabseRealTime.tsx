...
const useSupabaseRealtime = () => {
  ...
  useEffect(() => {
    const channel = supabase.channel('db-changes').on( 'postgres_changes', { event: '*', schema: 'public', table: 'files' }, async (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            if (
              ...
              dispatch({ type: 'ADD_FILE', payload: { file: newFile, folderId, workspaceId }});
            }
            ...
            if (fileExists && workspaceId && folderId) {
              ...
              dispatch({ type: 'DELETE_FILE', payload: { fileId: payload.old.id, folderId, workspaceId }});
            }
