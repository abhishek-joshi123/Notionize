...
const QuillEditor: React.FC<QuillEditorProps> = ({ dirDetails, dirType, fileId }) => {
  ...
  const details = useMemo(() => {
    let selectedDir;
    if(dirType == "file") selectedDir = state.workspaces.find((workspace) => workspace.id === workspaceId)?.folders.find((folder) => {folder.id === folderId})?.files.find((file) => {file.id === fileId})
    if(dirType === 'folder') selectedDir = state.workspaces.find((workspace) => workspace.id === workspaceId)?.folders.find((folder) => folder.id === fileId);
    if(dirType === 'workspace') selectedDir = state.workspaces.find((workspace) => workspace.id === fileId);
    ...
    const quillHandler = (delta: any, oldDelta: any, source: any) => {
      ...
      saveTimerRef.current = setTimeout(async () => {
        if (contents && quillLength !== 1 && fileId) {
          ...
          if (dirType == 'file') {
            if (!workspaceId || !folderId) return;
            dispatch({ type: 'UPDATE_FILE', payload: { file: { data: JSON.stringify(contents) }, workspaceId, folderId: folderId, fileId}});
  ...
  useEffect(() => {
    ...
    const subscription = room.on('presence', { event: 'sync' }, () => {
        ...
        if (!response) return;
        room.track({ id: user.id, email: user.email?.split('@')[0], avatarUrl: response.avatarUrl? supabase.storage.from('avatars').getPublicUrl(response.avatarUrl).data.publicUrl : '',
        });
      });
    return () => {
    ...
    <>
      ...
      {details.bannerUrl && (
        <div className="relative w-full h-[200px]">
          <Image src={supabase.storage.from('file-banners').getPublicUrl(details.bannerUrl).data.publicUrl} fill className="w-full md:h-48 h-20 object-cover" alt="Banner Image"/>
        </div>
      )}
