...
import { useSupabaseUser } from '@/lib/providers/supabse-user-provider';
...
const Dropdown:React.FC<DropdownProps> = ({title, listType, id, iconId, children, disabled, ...props}) => {
    ...
    const navigatatePage = (accordionId: string, type: string) => {
      ...
      if (type === 'file') router.push(`/dashboard/${workspaceId}/${folderId}/${accordionId.split('folder')[1]}`)
    };
    ...
    const handleBlur = async () => {
        ...
        if (fId.length === 2 && fId[1]) {
          ...
          if (error) toast({ title: 'Error', variant: 'destructive', description: 'Could not update the title for this file'});
          else toast({ title: 'Success', description: 'File title changed.'});
        }
      };
    ...
    const onChangeEmoji = async (selectedEmoji: string) => {
        ...
        if (listType === 'folder') {
          ...
          if (error) toast({ title: 'Error', variant: 'destructive', description: 'Could not update the emoji for this folder'});
          else toast({ title: 'Success', description: 'Update emoji for the folder'});
        }
      };
      ...
      const folderTitleChange = (e: any) => {
        if (!workspaceId) return;
        const fid = id.split('folder');
        if (fid.length === 1) dispatch({ type: 'UPDATE_FOLDER', payload: { folder: { title: e.target.value }, folderId: fid[0], workspaceId}});
      };

      const fileTitleChange = (e: any) => {
        if (!workspaceId || !folderId) return;
        const fid = id.split('folder');
        if (fid.length === 2 && fid[1]) dispatch({ type: 'UPDATE_FILE', payload: { file: { title: e.target.value }, folderId, workspaceId, fileId: fid[1]}});
      };
    ...
    const moveToTrash = async() => {
        ...
        if (listType === 'folder') {
            ...
            if (error) toast({ title: 'Error', variant: 'destructive', description: 'Could not move the folder to trash'});
            else toast({ title: 'Success', description: 'Moved folder to trash'});
        }
        if (listType === 'file') {
    ...
    const addNewFile = async () => {
        ...
        if (error) toast({ title: 'Error', variant: 'destructive', description: 'Could not create a file',});
        else toast({ title: 'Success', description: 'File created.'});
    };
    ...
    return (
      ...
      <AccordionContent>
        {state.workspaces.find((workspace) => workspace.id === workspaceId)?.folders.find((folder) => folder.id === id)?.files.filter((file) => !file.inTrash).map((file) => {
