...
const UserCard: React.FC<UserCardProps> = async ({ subscription }) => {
  ...
  else avatarPath = supabase.storage.from('avatars').getPublicUrl(response.avatarUrl)?.data.publicUrl
  ...
  return (
    ...
    <aside className="flex justify-center items-center gap-2">
        ...
        <div className="flex flex-col">
          ...
          <small className="w-[100px] overflow-hidden overflow-ellipsis">{profile.email}</small>
        </div>
      </aside>
